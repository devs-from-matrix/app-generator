...
const git = require('simple-git');
  ...
  assertOk(!_.isEmpty(userName), 'userName is required');

  // Validate template name
  const token = argv.g || argv.token;
  assertOk(!_.isEmpty(token), 'token is required');
...
gulp.task('create-repo', () => {
  const domainNameLowerCase = domainName.toLowerCase();
  const configs = {headers: {Authorization: 'Bearer ' + token, Accept: 'application/vnd.github.baptiste-preview+json'}};
  const repoUrl = `https://api.github.com/repos/${userName}/basic-template-repository/generate`;
  const data = { owner: userName, name : domainNameLowerCase, description: domainNameLowerCase, private: false };
  return Promise.all([
...
gulp.task('commit', () => {
const domainNameLowerCase = domainName.toLowerCase();
const repoUrl = `https://${userName}:${token}@github.com/${userName}/${domainNameLowerCase}.git`;
  return git(path.join(__dirname, 'generated', '/'))
    .init()
    .add('./*')
    .commit('feat: initial commit generated by app generator')
    .addRemote('origin', repoUrl)
    .push(['-u', 'origin', 'master'], () => console.log('done'))
});

gulp.task('default', function (done) {
  runSequence('cleanup', 'create-repo', 'generate', 'commit', done);
});
